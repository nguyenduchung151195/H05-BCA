...
function FilterDocs(props) {
    ...
    const [apiUrl, setApiUrl] = useState(API_INCOMMING_DOCUMENT)
    ...
    const mapFunction = item => {
        return {
            ...
            toBookCode: item.toBookCode ? moment(item.toBookCode).format('DD/MM/YYYY') : '',
            toBookDate: item.toBookDate ? moment(item.toBookDate).format('DD/MM/YYYY') : '',
            receiveDate: item.receiveDate ? moment(item.receiveDate).format('DD/MM/YYYY') : '',
    ...
    useEffect(()=>{
        if(profile){
            ...
            setApiUrl(`${API_INCOMMING_DOCUMENT}?${query}`)
        }
        else {
            setApiUrl(API_INCOMMING_DOCUMENT)
        }
    }, [])
    return (
        <div>
