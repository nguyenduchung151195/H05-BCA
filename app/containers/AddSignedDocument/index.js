...
function AddSignedDocument(props) {
  ...
  let id = props.match && props.match.params ? props.match.params.id : 'add';
  if (id && id.indexOf("?") !== -1) {
  ...
  const handleSave = useCallback(
    () => {
      ...
      localStateClone["drafter.name"] = localStateClone.drafter
      const messages = viewConfigCheckForm(code, localStateClone);
      console.log(messages, 'messages', Object.values(messages).filter(v => v))
  ...
  const handleDeleteIds = () => {
    ...
    recipientIds.splice(idDeletee, 1)
    recipientIds.map(el => {
  ...
  return (
    ...
    >
      <div style={{ width: "calc(100vw - 260px)" }} >
        ...
        <Dialog title="Tải file" onClose={() => setOpenDialogScan(false)} open={openDialogScan}>
          <ScanFileByH
        ...
        <CustomAppBar
          title={id ? 'Chỉnh sửa dự thảo văn bản trình ký' : 'Thêm mới dự thảo văn bản trình ký'}
          onGoBack={() => {
            setOpenCustom(false)
            onCloseProject();
          ...
          moreButtons={
            <div style={{ marginRight: 10, display: 'flex', justifyContent: 'space-around' }}>
              <Button
                ...
                onClick={() => setOpenDialogScan(true)}
              >
                Nhập liệu
              </Button>
              {(isDone || isDoneAuthor) && (
              ...
              {(canFeedBack || (canFeedBack && isAuthory)) && (
                <Button
                  style={{ marginRight: 10 }}
                  variant="outlined"
                  color="inherit"
              ...
              {role !== 'receive' &&
                ((checkSetProcessor() && isEndFlow) || (isAuthory && checkSetProcessor() && isEndFlow)) && (
                  <Button
                    variant="outlined"
                    color="inherit"
                    onClick={e => {
            ...
            <Grid container spacing={8}>
              ...
              <Grid item xs={3}>
                <CustomInputBase
                  ...
                  onChange={e => handleInputChange(e)}
                  required={checkRequired.recipientsOutSystem}
                  checkedShowForm={checkShowForm.recipientsOutSystem}
                  error={localMessages && localMessages.recipientsOutSystem}
                  helperText={localMessages && localMessages.recipientsOutSystem}
                />
              </Grid>
        ...
        <Dialog dialogAction={false} onClose={closeDialogTask} open={openDialogTask}>
          <TaskDocs onSave={onSaveTask} handleCloseFilter={closeDialogTask} profile={props.profile} />
        </Dialog>
