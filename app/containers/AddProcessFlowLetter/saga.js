...
export function* fetchApproveGroupDetailPage(action) {
  try {
    const data = yield call(requestApprove, `${API_DOCUMENT_PROCESS_TEMPLATE}/${action.id}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
      },
...
export function* fetchUpdateApproveGroups(action) {
  ...
  try {
    const updateApproveGroups = yield call(requestApprove, `${API_DOCUMENT_PROCESS_TEMPLATE}/${action.doc._id}`, {
      method: 'PUT',
      headers: {
...
export function* fetchAddApproveGroup(action) {
  ...
  try {
    ...
    action.doc.type = localStorage.getItem('processFlowType');
    const addApproveGroup = yield call(requestApprove, `${API_DOCUMENT_PROCESS_TEMPLATE}`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
