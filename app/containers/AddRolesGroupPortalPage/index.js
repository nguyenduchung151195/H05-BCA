...
import makeSelectAddRolesGroupPortalPage from './selectors';
import reducer from './reducer';
import saga from './saga';
import { getModuleAct, addRoleGroupPortal, getInforRoleGroupPortalAction, editRoleGroupPortalAct } from './actions';
...
export class AddRolesGroupPortalPage extends React.Component {
  state = {
    roleName: '',
    roleDes: '',
    code: '',
  ...
  componentWillReceiveProps(props) {
    if (props !== this.props) {
      // console.log(props);
      const { addRolesGroupPortalPage } = props;
      const modules = addRolesGroupPortalPage.modules || [];
      const roleGroup = addRolesGroupPortalPage.roleGroup;
      let { roles } = roleGroup;
      const { match } = this.props;
      if (!match.params.id) {
        roles = [];
      ...
      if (
        roleGroup !== null &&
        props.addRolesGroupPortalPage.roleGroup !== this.props.addRolesGroupPortalPage.roleGroup &&
      ...
      if ((this.props.addRolesGroupPortalPage ? this.props.addRolesGroupPortalPage.successCreate : false) === true) {
    ...
    return (
      <div>
        ...
        <CustomAppBar
          title={
            addStock === 'add'
              ? `Thêm mới vai trò Portal`
              : `Cập nhật vai trò Portal`
          }
              ...
              <Checkbox
                onClick={() => {
                  ...
                  if (applyEmployeeOrgToModuleOrg) {
                    const { addRolesGroupPortalPage } = this.props;
                    let listOrganizationUnit = addRolesGroupPortalPage.listOrganizationUnit || [];
  ...
  componentDidUpdate() {
    const { addRolesGroupPortalPage } = this.props
    if (addRolesGroupPortalPage
      && addRolesGroupPortalPage.loading === false
      && addRolesGroupPortalPage.success === false
      && addRolesGroupPortalPage.successCreate === false
      && addRolesGroupPortalPage.error === true
...
AddRolesGroupPortalPage.propTypes = {
...
const mapStateToProps = createStructuredSelector({
  addRolesGroupPortalPage: makeSelectAddRolesGroupPortalPage(),
});
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    onAddRoleGroup: (body, roles, newAllowedDepartment) => {
      dispatch(addRoleGroupPortal(body, roles, newAllowedDepartment));
    },
    onGetInfor: id => {
      dispatch(getInforRoleGroupPortalAction(id));
    },
    onChangeSnacker: obj => dispatch(changeSnackbar(obj)),
    onEditRoleGroup: body => {
      dispatch(editRoleGroupPortalAct(body));
    },
  };
}
...
const withReducer = injectReducer({ key: 'addRolesGroupPortalPage', reducer });
const withSaga = injectSaga({ key: 'addRolesGroupPortalPage', saga });
