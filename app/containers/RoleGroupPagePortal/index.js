 ...
 */
 ...
 import makeSelectRoleGroupPagePortal from './selectors';
 import reducer from './reducer';
 import saga from './saga';
 import styles from './styles';
 import { getRoleGroupPortalAction, deleteRoleGroupPortalAct, defaultAction } from './actions';
 ...
 let allId = [];
 export class RoleGroupPagePortal extends React.Component {
   constructor(props) {
     super(props);
     this.timeout = 0;
     this.state = {
   ...
   componentDidUpdate(preProps, preState) {
     ...
     else if (this.state.count === 0 && this.props && this.props.roleGroupPagePortal && this.props.roleGroupPagePortal.success === true) {
       this.setState({ roleData: this.props.roleGroupPagePortal.roleGroups, roleDataTemp: this.props.roleGroupPagePortal.roleGroups, count: this.state.count + 1 })
     }
     else if (preProps.roleGroupPagePortal && this.props.roleGroupPagePortal && preProps.roleGroupPagePortal !== this.props.roleGroupPagePortal) {
       this.setState({ roleData: this.props.roleGroupPagePortal.roleGroups, roleDataTemp: this.props.roleGroupPagePortal.roleGroups })
     }
   }
   ...
   componentWillReceiveProps(props) {
     if (props.roleGroupPagePortal.successDelete) {
   ...
   render() {
     ...
     const { classes, roleGroupPagePortal, intl } = this.props;
     // console.log(roleGroupPagePortal.roleGroups);
     ...
     const newRows = _.chunk(
       _.map(roleData, row => {
         const action = (
           <Fab
             component={NavLink}
             to={`/setting/roleGroupPortal/edit/${row._id}`}
             size="small"
             title="Chỉnh sửa"
             style={{ marginLeft: 10 }}
             color="primary"
     ...
     return (
       <div>
         ...
         <Helmet>
           ...
           <meta name="description" content="Description of RoleGroupPagePortal" />
         </Helmet>
         ...
         <GridUI container>
           ...
           <GridUI item md={12} xs={12} style={{ paddingRight: 30, padding: 10 }} justifyContent="flex-end" >
             ...
             <Fab>
               <Link to="/setting/roleGroupPortal/add">
         ...
         <Paper className={classes.tableContainer} id="table-full-width">
           ...
           <TablePagination
             ...
             component="div"
             count={roleGroupPagePortal.roleGroups.length}
 ...
 RoleGroupPagePortal.propTypes = {
   // dispatch: PropTypes.func.isRequired,
   classes: PropTypes.object.isRequired,
 };
 
 const mapStateToProps = createStructuredSelector({
   roleGroupPagePortal: makeSelectRoleGroupPagePortal(),
 });
 
 function mapDispatchToProps(dispatch) {
   return {
     dispatch,
     onGetRoleGroup: (filter) => dispatch(getRoleGroupPortalAction(filter)),
     onDelete: body => dispatch(deleteRoleGroupPortalAct(body)),
 ...
 const withReducer = injectReducer({ key: 'roleGroupPagePortal', reducer });
 const withSaga = injectSaga({ key: 'roleGroupPagePortal', saga });
