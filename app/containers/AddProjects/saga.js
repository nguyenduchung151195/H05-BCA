...
function* postProject(action) {
  ...
  try {
    ...
    // }
    const url = docSeen ? `${API_TASK_PROJECT}?docSeen=${docSeen}` : API_TASK_PROJECT
    ...
    if (action.res && action.res.isProject === true)
    ...
    yield put(changeSnackbar({ status: true, message: error.message || error.error || 'Thêm mới thất bại', variant: 'error' }));
  }
}
