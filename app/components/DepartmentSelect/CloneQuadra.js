...
import { API_ORIGANIZATION, API_PERSONNEL_RECEIVER } from '../../config/urlConfig';
...
function DepartmentSelect(props) {
  const { onChange, title, allowedDepartmentIds, allowedUsers = [], isMultiple, onChangeAddRoles, byOrg, handleChangeUserUnit, recipientIds, tempUnit } = props;
  const [data, setData] = useState([]);
  const [departments, setDepartments] = useState([]);
  const [count, setCount] = useState(0);
  const [recipientIdss, setRecipientIdss] = useState([]);
  ...
  //   tempUnit.map((el) => {
  //     console.log(el, "el")
  //     dataa.push(el.recipientId)
  //   })
  //   setRecipientIdss(dataa)
  // }, [tempUnit]);
  useEffect(
    () => {
  ...
  const getData = async () => {
    try {
      ...
      let flattedDepartment = flatChild(departmentsRes);
  ...
  function handleCheckDerpartment(name, valueName, checked) {
    try {
      ...
      setData(newData);
      let viewedDepartmentIds = newData.filter(item => item.data.view).map(item => item.name);
  ...
  const getReceivers = (recipientId) => {
    let data = ""
    tempUnit.map((el) => {
      if (el.recipientId === recipientId) data = el.receivers
    })
    return data
    // value={tempUnit && tempUnit[allowedDepartmentIds.indexOf(data[rowIndex].id)] && tempUnit[allowedDepartmentIds.indexOf(data[rowIndex].id)].receivers || ""}
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell>Cán bộ nhận</TableCell>
                        ...
                        <>
                          ...
                          <TableCell>
                            <Grid item style={{ minWidth: 200 }}>
                              ...
                              {!!(data[rowIndex] && data[rowIndex].data && data[rowIndex].data[it.name]) && <AsyncAutocomplete
                                disabled={!!!(data[rowIndex] && data[rowIndex].data && data[rowIndex].data[it.name])}
                                filter={{ ['organizationUnit.organizationUnitId']: data[rowIndex].id }}
                                onChange={e => {
                                  handleChangeUserUnit(data[rowIndex].id, e)
                                  setCount(count + 1)
                                }}
                                value={
                                  getReceivers(data[rowIndex].id)
                                }
                                label="Cán bộ nhận"
                                limit="5000"
