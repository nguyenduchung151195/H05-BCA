...
function DepartmentSelect(props) {
  ...
  useEffect(
    () => {
      ...
      if (docIds !== 'add') {
        ...
        const dataTaskFm = historyTask && historyTask.length > 0 && historyTask.map(i => i.receiver._id);
  ...
  function disabledInChage(it) {
    ...
    if (docIds === 'add') {
      ...
      if (it.name === 'processing' && disableEditProcessing && disableEditProcessing.length > 0) {
        return true;
      } else {
        return false;
      }
