...
import { API_INCOMMING_DOCUMENT, API_USERS, API_ORIGANIZATION, API_ROLE_APP, API_DOCUMENT_HISTORY, MIN_ORDER, API_OUTGOING_DOCUMENT_RETURN_DOCS } from '../../config/urlConfig';
...
function DepartmentSelect(props) {
  ...
  useEffect(
    () => {
      getData();
      if (open) {
        let urlListUser = API_OUTGOING_DOCUMENT_RETURN_DOCS;
        fetchData(`${urlListUser}?docId=${docIds}`)
          .then(response => {
            if (response && response.data) {
              ...
              //     if (el.highlight) setSearch(el.name)
              //   })
            }
          })
          .catch(e => {
    ...
    const x = departments.map((i, index) => {
      ...
      const departmentUsers = (
        ...
        .map(u => {
          ...
          if (!checkFirst && returnTypeDocument === "OutgoingDocument" && u.highlight) {
            setCheckFirst(1)
  ...
  const getName = (item) => {
    ...
    if (roleAcc && roleAcc.order <= MIN_ORDER) {
      // return `${item.nameAuthorized || item.userNameCopy} - ${roleAcc && roleAcc.name || " "}` || item.nameAuthorized || item.userNameCopy
      let name = ""
      if (item && item.nameAuthorized) {
                  ...
                  {i.expand && data[rowIndex] && data[rowIndex].users && data[rowIndex].users.length
                    ? sortAZ(data[rowIndex].users).map(user => (
                      <>
                        ...
                        <Grid container className={user.highlight ? "customHighlight" : "customHover"} style={{ justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e2e2' }}>
                          ...
                          {columns.map(it => (
                            <Grid item style={{ display: 'flex', justifyContent: 'center', maxWidth: '100px', width: '100px' }}>
                              <>
                                <Checkbox
                                  // disabled
                                  // checked={user && user.highlight || false}
                                  checked={(user && user.data && user.data[it.name]) || (isFirstID && Array.isArray(isFirstID) && isFirstID[0] === user._id) || false}
                                  onChange={() => {
