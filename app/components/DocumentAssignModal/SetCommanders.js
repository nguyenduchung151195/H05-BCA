...
const DocumentAssignModal = props => {
  const { open, docIds, onChangeSnackbar, template, currentRole, isAuthory = false, roleGroupSource, consult, profile } = props;
  ...
  return (
    <DialogUI
      title="Xin ý kiến"
      disableSave={!result || !result.commandersUsers || !docIds || !docIds.length || disableSave}
      onSave={() => {
        setDisableSave(true);
