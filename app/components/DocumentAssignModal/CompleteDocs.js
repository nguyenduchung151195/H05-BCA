...
const DocumentAssignModal = props => {
  const { open, docIds, onChangeSnackbar, template, currentRole, typePage = "", isAuthory = false, flagFlashComplete} = props;
  ...
  return (
    <DialogUI title="Hoàn thành xử lý" onSave={() => {
      setDisableSave(true)
      let url;
      if (isAuthory) {
        url = props.isLetter ? `${API_LETTER}/set-complete?authority=true` : `${API_INCOMMING_DOCUMENT}/set-complete`;
      } else {
        url = props.isLetter ? `${API_LETTER}/set-complete` : `${API_INCOMMING_DOCUMENT}/set-complete`;
      }

      url = typePage !== '' ? `${url}?authority=true` : url;
      const body = {
        docIds,
        note,
        processResult: resultProcess,
        saveResult: resultSave,
        cmd: flagFlashComplete
      };
      // fetchData(url, {
      //   method: 'POST',
      //   headers: {
      ...
      .then(res => {
        ...
        setDisableSave(false)
        onChangeSnackbar({ variant: 'error', message: error.message || 'Hoàn thành xử lý kiến thất bại', status: true })
      })
    }} saveText="Hoàn thành" disableSave={!docIds || !docIds.length || disableSave} onClose={() => props.onClose && props.onClose()} open={open} style={{ position: 'relative' }}>
      {/* <DepartmentSelect
        title=""
