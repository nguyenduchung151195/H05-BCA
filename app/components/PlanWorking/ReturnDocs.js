...
const DocumentAssignModal = props => {
  const { open, docIds, onChangeSnackbar, type = '', processeds = [], processors = [], template, childTemplate, role, currentRole, isAuthory = false } = props;
    ...
    templates.map(temp => {
      if (temp.children) {
        if (child) {
          ...
          index = temp && temp.children && temp.children.findIndex(f => f.idTree == item.idTree);
          if (index !== -1) {
            if (temp.children) {
              if (roleSelect) {
