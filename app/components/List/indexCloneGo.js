...
const ExportTable = React.memo(
  ({ filters, filter, filter1, filter2, open, onClose, apiUrl, moduleCode, kanban, viewConfigUpdateTime, exportUrl }) => {
    ...
    const [viewConfigData, setViewConfigData] = useState([
      { name: 'nameOrg', order: 1, title: 'Tên đơn vị', title_init: 'Tên đơn vị', type: 'String', width: 150 },
      { name: 'count', order: 2, title: 'Tổng số văn bản', title_init: 'Tổng số văn bản', type: 'String', width: 50 },
      { name: 'countSignDigitalDoc', order: 3, title: 'Văn bản kí số', title_init: 'Văn bản kí số', type: 'String', width: 50 },
      { name: 'countSignTextDoc', order: 4, title: 'Văn bản kí nháy', title_init: 'Văn bản kí nháy', type: 'String', width: 50 },
      { name: 'countPromulgateDoc', order: 5, title: 'Ban hành', title_init: 'Ban hành', type: 'String', width: 50 },
      { name: 'countCommandDoc', order: 6, title: 'Thừa lệnh', title_init: 'Thừa lệnh', type: 'String', width: 50 },
    ]);
...
class ListPage extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    // });
    let listName = [];
    this.props.optionSearch.map((el) => {
        ...
        <GridContainer>
...

          {
            !this.state.openSearch && <>
              <GridCore item md={12} style={{ display: 'flex', alignItems: 'center', justifyContent: "end", marginTop: 5, marginBottom: 5 }}>
                <Fab onClick={e => {
                  this.props.setOpenSearch && this.props.setOpenSearch(!this.state.openSearch)
                  this.setState({ openSearch: !this.state.openSearch })
                }}>
                  <Tooltip title={this.state.openSearch ? "Đóng lại" : "Mở ra"}>
                    {
                      this.state.openSearch ? <ExpandMore /> : <ExpandLess />
                    }
                  </Tooltip>
                </Fab>
              </GridCore>
          ...
          {!this.props.disableFilter ? this.state.openSearch && (
            <>
              {this.props.reports ? (
                <GridItem item xs={2}>
...
const GridList = React.memo(
  ({
    ...
    return (
      <div ref={containerRef} style={{ width: '100%' }} className="ktCustom ktCustom2">
...
const MenuFilter = React.memo(({ anchorEl, closeFilter, columns, selectField, filters, code, optionSearch }) => {
  if (code) {
    ...
    const data = list && list.listDisplay && list.listDisplay.type.fields.type.columns;
    const others = list && list.listDisplay && list.listDisplay.type.fields.type.others;
    let dataSearch = data && data.filter(item => item.type === 'String' && item.name === 'toBook');
    if (optionSearch) {
