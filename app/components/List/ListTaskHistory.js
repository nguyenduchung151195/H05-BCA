...
class ListPage extends React.PureComponent {
  ...
  getData = dt => {
    ...
    const { mapFunction, customFunction, apiUrl, disableDot, client, filter, filterCan, noQuery, isUserPage = false } = this.props;
    ...
    let URL;
    if (apiUrl.includes('?')) {
      URL = noQuery ? `${apiUrl}` : filterCan ? `${apiUrl}&${queryClientCan}` : client ? `${apiUrl}&${queryClient}` : `${apiUrl}&${query}`;
    } else {
      URL = noQuery ? `${apiUrl}` : filterCan ? `${apiUrl}?${queryClientCan}` : client ? `${apiUrl}?${queryClient}` : `${apiUrl}?${query}`;
    }
    ...
    fetch(URL, {
      ...
      .then(async data => {
        ...
        newList = newList.map(it => {
          ...
          return {
            ...it,
            edit: this.addEdit(it._id, it, enableEdit),
            state: result,
            kanbanStatus: !this.props.noKanban ? kbStatus : it.kanbanStatus,
          };
        });
        if (isUserPage === false) {
...
const GridList = React.memo(
  ({
    ...
    const dataFmtask1 = rows.filter(i => i.statusAccept === 'Chờ xác nhận');
    ...
    return (
      <div ref={containerRef} style={{ width: '100%' }}>
        ...
        <Paper md={12} style={{ overflow: 'auto', boxShadow: 'none' }}>
          {true ? (
            <Grid rows={rows} columns={newCls}>
              ...
              />
              {tree ? null : disableSelect ? null : dataFmtask1 ? <TableSelection showSelectAll={false} /> : null}
      ...
      <div>
        {othersButton ? (
          ...
          <MenuItem onClick={handleApprove}>Yêu cầu phê duyệt</MenuItem> */}
            ...
            {code === 'IncommingDocument' && <MenuItem onClick={handleCreateOutGoingDoc}>Tạo dự thảo</MenuItem>}
