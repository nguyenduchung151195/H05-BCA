...
const ExportTable = React.memo(
  ({ filters, filter, filter1, filter2, open, onClose, apiUrl, moduleCode, kanban, viewConfigUpdateTime, exportUrl, onChangeSnackbar , queryStr}) => {
    ...
    const [viewConfigData, setViewConfigData] = useState([
      ...
      { name: "toBook", order: 2, title: "Số văn bản", title_init: "Số văn bản", type: "String", width: 50 },
      { name: "documentDate", order: 3, title: "Ngày văn bản", title_init: "Ngày văn bản", type: "String", width: 50 },

      { name: "senderUnit", order: 4, title: "Đơn vị gửi", title_init: "Đơn vị gửi", type: "String", width: 50 },
      { name: "abstractNote", order: 5, title: "Trích yếu", title_init: "Trích yếu", type: "String", width: 50 },
      { name: "deadline", order: 6, title: "Hạn xử lý", title_init: "Hạn xử lý", type: "String", width: 50 },
      { name: "documentType", order: 7, title: "Loại văn bản", title_init: "Loại văn bản", type: "String", width: 50 },

      { name: "privateLevel", order: 8, title: "Độ mật", title_init: "Độ mật", type: "String", width: 50 },
      { name: "urgencyLevel", order: 9, title: "Độ khẩn", title_init: "Độ khẩn", type: "String", width: 50 },
      { name: "files", order: 10, title: "File đính kèm", title_init: "File đính kèm", type: "String", width: 50 },
    ]);
    ...
    useEffect(
      () => {
        if (moduleCode) {
          ...
          if (moduleCode === "IncommingDocument") {
            setViewConfigData([
              { name: "stt", order: 1, title: "STT", title_init: "STT", type: "String", width: 200 },
              { name: "toBook", order: 2, title: "Số văn bản", title_init: "Số văn bản", type: "String", width: 200 },
              { name: "documentDate", order: 3, title: "Ngày văn bản", title_init: "Ngày văn bản", type: "String", width: 200 },
              { name: "senderUnit", order: 4, title: "Đơn vị gửi", title_init: "Đơn vị gửi", type: "String", width: 200 },
              { name: "abstractNote", order: 5, title: "Trích yếu", title_init: "Trích yếu", type: "String", width: 200 },
              { name: "deadline", order: 6, title: "Hạn xử lý", title_init: "Hạn xử lý", type: "String", width: 200 },
              { name: "documentType", order: 7, title: "Loại văn bản", title_init: "Loại văn bản", type: "String", width: 200 },
              { name: "privateLevel", order: 8, title: "Độ mật", title_init: "Độ mật", type: "String", width: 200 },
              { name: "urgencyLevel", order: 9, title: "Độ khẩn", title_init: "Độ khẩn", type: "String", width: 200 },
              { name: "files", order: 10, title: "File đính kèm", title_init: "File đính kèm", type: "String", width: 200 },
            ])
          }
          else if (moduleCode === "OutGoingDocument") {
            setViewConfigData([
              { name: "stt", order: 1, title: "STT", title_init: "STT", type: "String", width: 200 },
              { name: "releaseNo", order: 2, title: "Số VB đi", title_init: "Số VB đi", type: "String", width: 200 },
              { name: "abstractNote", order: 6, title: "Trích yếu", title_init: "Trích yếu", type: "String", width: 200 },
              { name: "toBook", order: 3, title: "Số ký hiệu văn bản", title_init: "Số ký hiệu văn bản", type: "String", width: 200 },
              { name: "documentType", order: 4, title: "Loại văn bản", title_init: "Loại văn bản", type: "String", width: 200 },
              { name: "privateLevel", order: 8, title: "Độ mật", title_init: "Độ mật", type: "String", width: 200 },
              { name: "urgencyLevel", order: 9, title: "Độ khẩn", title_init: "Độ khẩn", type: "String", width: 200 },
              { name: "createdBy", order: 5, title: "Người tạo", title_init: "Người tạo", type: "String", width: 200 },
              { name: "documentType", order: 7, title: "Người soạn thảo", title_init: "Người soạn thảo", type: "String", width: 200 },
              { name: "drafter.name", order: 10, title: "File đính kèm", title_init: "File đính kèm", type: "String", width: 200 },
            ])
          }
        }
      },
    ...
    const getDataFirstTime = async () => {
      try {
        ...
        viewConfigData.forEach(item => {
          if (item.title !== "STT") arr.push(item.name);
        });
        ...
        let res
        if(queryStr)
         res = await fetchData(apiUrl, "POST", {filter: queryStr});
         else {
          res = await fetchData(apiUrl);
         }
        if (!res.data || !res) {
        ...
        const numberRecordLimitBackEnd = parseInt(res.limit);
        if (res.count > numberRecordLimitBackEnd) {
          ...
          if (data && Array.isArray(data) && data.length) {
            data.map((el) => {
              dataCopy.push({
                ...el,
                deadline: el.deadline ? moment(el.deadline).format("DD/MM/YYYY") : ""
              })
            })
          }
          console.log(data, 'data')

          setDataPageExcell({ ...dataPageExcell, data: dataCopy, totalPage: 1, pageNumber: 1, numberOrderFirstPage: 1 });
          onClose({ lastPage: true });
        }
        // setDataPageExcell({ ...dataPageExcell, data: res.data, totalPage: 1, pageNumber: 1, numberOrderFirstPage: 1 });
        // onClose({ lastPage: true });
      } catch (err) {
        console.log(err, "djfnv")
    ...
    const getDataPagination = async (skip, limit, totalPage, pageNumber) => {
      ...
      try {
        ...
        setDataPageExcell({ ...dataPageExcell, data: dataCopy, totalPage: totalPage, pageNumber: pageNumber, numberOrderFirstPage: skip + 1 });
    ...
    return (
      <React.Fragment>
        {open ? <Loading /> : null}
        <div id={`${moduleCode + kanban}s`} style={{ display: 'none' }}>
          <table>
            <tbody>
              <tr>
                <td />
...
class ListPage extends React.PureComponent {
  ...
  handleCloseExportTable = payload => {
    ...
    switch (openExport) {
      case 'PDF':
        ...
        break;
      case 'Excel':
        tableToExcelCustom(`${this.props.code}${this.props.kanban}s`, 'W3C Example Table', this.props.code, "", "", 8);
        break;
      default:
        break
  ...
  getData = dt => {
    ...
    const method = queryStr ? "POST" : "GET"
    console.log(method, 'method')
    if(queryStr){
// tra cứu thống kê văn bản đến
fetch(URL, {
  method: method,
  headers: {
  ...
  body: JSON.stringify({filter: queryStr})
})
  ...
  .then(async data => {
    ...
    if (client) {
      ...
      let rowss = []
      let columnExtensions = this.props.columnExtensions
      newList.map((el, index) => {
        // if (el.files && el.files.length > lengthFile) lengthFile = el.files.length
                      ...
                      <GridItem sm={3} style={{ position: 'relative', marginBottom: 5 }} custom="customLeft">
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          <CustomDatePicker
                            ...
                            onChange={e => this.setState({ filterDate_startDate: e && e.format('YYYY-MM-DD') })}
                            required={false}
                            maxDateMessage=""
                            allowRemove2
                            minDateMessage=""
                      ...
                      <GridItem sm={3} style={{ position: 'relative', marginBottom: 5 }} custom="customLeft">
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          <CustomDatePicker
                            ...
                            onChange={e => this.setState({ filterDate_endDate: e && e.format('YYYY-MM-DD') })}
                            required={false}
                            maxDateMessage=""
                            allowRemove2
                            minDateMessage=""
  ...
  ({
    ...
    const [newCls, setNewCls] = React.useState(
      code === "IncommingDocument" ? [
        { name: "stt", order: 1, title: "STT", title_init: "STT", type: "String", width: 10 },
        { name: "toBook", order: 2, title: "Số văn bản", title_init: "Số văn bản", type: "String", width: 200 },
        { name: "documentDate", order: 3, title: "Ngày văn bản", title_init: "Ngày văn bản", type: "String", width: 200 },
        { name: "senderUnit", order: 4, title: "Đơn vị gửi", title_init: "Đơn vị gửi", type: "String", width: 200 },
        { name: "abstractNote", order: 5, title: "Trích yếu", title_init: "Trích yếu", type: "String", width: 200 },
        ...
        { name: "privateLevel", order: 8, title: "Độ mật", title_init: "Độ mật", type: "String", width: 200 },
        { name: "urgencyLevel", order: 9, title: "Độ khẩn", title_init: "Độ khẩn", type: "String", width: 200 },
        { name: "files", order: 10, title: "File đính kèm", title_init: "File đính kèm", type: "String", width: 500 },
      ] :
        [
          { name: "stt", order: 1, title: "STT", title_init: "STT", type: "String", width: 10 },
          { name: "releaseNo", order: 2, title: "Số VB đi", title_init: "Số VB đi", type: "String", width: 200 },
          { name: "abstractNote", order: 3, title: "Trích yếu", title_init: "Trích yếu", type: "String", width: 500 },
          { name: "toBook", order: 4, title: "Số ký hiệu văn bản", title_init: "Số ký hiệu văn bản", type: "String", width: 200 },
          { name: "documentType", order: 5, title: "Loại văn bản", title_init: "Loại văn bản", type: "String", width: 200 },
          { name: "privateLevel", order: 6, title: "Độ mật", title_init: "Độ mật", type: "String", width: 200 },
          { name: "urgencyLevel", order: 7, title: "Độ khẩn", title_init: "Độ khẩn", type: "String", width: 200 },
          { name: "createdBy", order: 8, title: "Người tạo", title_init: "Người tạo", type: "String", width: 200 },
          { name: "drafter.name", order: 9, title: "Người soạn thảo", title_init: "Người soạn thảo", type: "String", width: 200 },
          { name: "files", order: 10, title: "File đính kèm", title_init: "File đính kèm", type: "String", width: 500 },
        ]
      ...
      () => {
        ...
        setDefaultColumnWidths(
          ...
          newCls.map(item => ({
            ...
            width: parseInt(item.width) > 200 ? item.width : 120,
          })),
        );
      },
      [columns],
...
ListPage.defaultProps = {
  ...
  filter: { status: 1 },
  treeName: 'name',
  columnExtensions: [{ columnName: 'abstractNote', width: 300 }, { columnName: 'files', width: 300 }],
  rightColumns: ['edit'],
  reload: false,
  perPage: 50,
  status: 'crmStatus',
