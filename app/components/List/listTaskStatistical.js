...
const ExportTable = React.memo(
  ({ filters, filter, filter1, filter2, open, onClose, apiUrl, moduleCode, kanban, viewConfigUpdateTime, exportUrl, noSelectors , exportFile, customDataExport}) => {
    ...
    const [viewConfigData, setViewConfigData] = useState([
      ...
      { name: 'name', order: 2, title: 'Tên công việc', title_init: 'Tên công việc', type: 'String', width: 50 },
      { name: 'description', order: 3, title: 'Nội Dung công việc', title_init: 'Nội Dung công việc', type: 'String', width: 50 },
      { name: 'taskMaster', order: 4, title: 'Người giao việc', title_init: 'Người giao việc', type: 'String', width: 50 },
      { name: 'inCharge', order: 5, title: 'Cán bộ xử lý', title_init: 'Cán bộ xử lý', type: 'String', width: 50 },
      { name: 'support', order: 6, title: 'Cán bộ phối hợp', title_init: 'Cán bộ phối hợp', type: 'String', width: 50 },
      { name: 'priority', order: 7, title: 'Độ ưu tiên', title_init: 'Độ ưu tiên', type: 'String', width: 50 },
      { name: 'startDate', order: 8, title: 'Ngày giao việc', title_init: 'Ngày giao việc', type: 'String', width: 50 },
      { name: 'endDate', order: 9, title: 'Ngày kết thúc', title_init: 'Ngày kết thúc', type: 'String', width: 50 },
    ]);
    ...
    const getDataFirstTime = async () => {
      try {
        ...
        if(exportFile && apiUrl.includes("limit")){
          // const idxLimit = apiUrl.indexOf("limit")
          // const afterLimit = apiUrl.slice(idxLimit)
          // const idxConnect = apiUrl.indexOf("&")
          // apiUrl= `${apiUrl.slice(0, idxLimit)}${apiUrl.slice(idxConnect + 1)}`
          // apiUrl=apiUrl.replace("")
          if (apiUrl.includes('?')) {
            apiUrl = `${apiUrl}&exportFile=true`;
          } else {
            apiUrl = `${apiUrl}?exportFile=true`;
          }
        }
        ...
        const numberRecordLimitBackEnd = res.limit || exportFile ? res.count : 50;
    ...
    const getDataPagination = async (skip, limit, totalPage, pageNumber) => {
      ...
      try {
       ...
       console.log(apiUrl, "apiUrl apiUrl")
        ...
        await onClose({ totalPage, pageNumber, lastPage });
      } catch (err) {
        console.log(err, "err")
...
class ListPage extends React.PureComponent {
      ...
      fetchData(`${API_ROLE_APP}/${this.props.code}/${this.props.profile._id}`).then(res => {
        ...
        if (foundRoleDepartment) {
          ...
          if (this.props.rootTab && data && Array.isArray(data) && data.length > 0) {
            ...
            if (roottab === 'processors') roottab = 'processing';
            else if (roottab === 'supporters') roottab = 'support';
            else if (roottab === 'viewers') roottab = 'view';
            else if (roottab === 'commander') roottab = 'command';
  ...
  getDataAsynce = async (code, kanbanList) => {
    ...
    if (view) {
      ...
      let newCls = [...columns, ...others];

      let columnOrder = newCls
      ...
      //     showInReport: true,
      //     title_init: 'Người giao việc',
      //     type: 'ObjectId|OrganizationUnit',
      //     version: 1,
      //     width: 180,
      //   },
      //   {
      ...
      //     order: 10,
      //     showInReport: true,
      //     title: 'Cán bộ phối hợp',
      //     showInReport: true,
      //     title_init: 'Cán bộ phối hợp',
      //     type: 'ObjectId|hrm',
      //     version: 1,
      //     width: 180,
      //   },
      // ];
      this.setState({ columns: [], columnOrder, fileColumns, loading: true, kanbanList, filterField: view.filterField }, () => {
        if (this.props.client) {
          this.getData({ loading: false });
        } else {
          this.loadData();
  ...
  handleCloseExportTable = async (payload) => {
    ...
    switch (openExport) {
      case 'PDF':
        ...
        const content = await tableToPDF(`${this.props.code}${this.props.kanban}`, this.props.code);
        this.setState({ html: [...this.state.html, { content, pageNumber }], htmlTotal: totalPage });
        break;
      default:
        tableToExcelCustom(`${this.props.code}${this.props.kanban}`, 'W3C Example Table', this.props.code, '', 'Tra cứu thống kê', 5);
    }
  };
  ...
  queryString = () => {
    ...
    const rex = `${search}`;
    if (search) {
      // filter.$or = filters.map(i => i === 'toBook' && { [i]: { $regex: rex, $options: 'gi' } });

      filter.$or = filters.map(i => i === 'toBook' && { [i]: rex });
      // const filterr = filters.map(i => {
      //   if (i === 'toBook') return { [i]: rex };
      // });
      // filter = { filter, ...filterr };
      filter = { ...filter };
    }
      ...
      <>
        <GridContainer>
                  ...
                  {disableSearch ? null : client ? (
                    ...
                    <GridItem md={12} sm={12} style={{ display: 'flex', alignItems: 'center' }} container>
                      {this.props.hideSearch ? null : this.props.notChangeSearch ? (
                        ...
                        <>
                          <GridItem xs={2} style={{ marginLeft: -5 }}>
                            <TextFieldUI
                              variant="outlined"
                              InputProps={{
                                endAdornment: (
                      ...
                      {this.props.filterStartEnd && (
                        <>
                          <GridItem sm={3} md={2} style={{ position: 'relative' }}>
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                              <CustomDatePicker
                                label="Từ ngày"
                          ...
                          <GridItem sm={3} md={2} style={{ position: 'relative', marginBottom: 5, marginLeft: -10, marginRight: 10 }} custom="customLeft">
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                              <CustomDatePicker
                                label="Đến ngày"
                                InputLabelProps={{ style: { fontSize: 14 }, shrink: true }}
                      ...
                      <GridItem xs={12} md={12} container>
                        <GridItem xs={6} md={6} style={{ display: 'flex', alignItems: 'center', justifyContent: 'end' }}>
                          <Button
                            ...
                            onClick={() => {
                              this.onFilterSearch();
                              this.handleSearch(this.state.dataSearch);
          ...
          <ExportTable
            ...
            noSelectors={this.props.noSelectors}
...
const GridList = React.memo(
  ({
    ...
    const [newCls, setNewCls] = React.useState([
      ...
      { name: 'name', order: 2, title: 'Tên công việc', title_init: 'Tên công việc', type: 'String', width: 50 },
      { name: 'description', order: 3, title: 'Nội Dung công việc', title_init: 'Nội Dung công việc', type: 'String', width: 50 },
      { name: 'taskMaster.name', order: 4, title: 'Người giao việc', title_init: 'Người giao việc', type: 'String', width: 50 },
      { name: 'inCharge', order: 5, title: 'Cán bộ xử lý', title_init: 'Cán bộ xử lý', type: 'String', width: 50 },
      { name: 'support', order: 6, title: 'Cán bộ phối hợp', title_init: 'Cán bộ phối hợp', type: 'String', width: 50 },
      { name: 'priority', order: 7, title: 'Độ ưu tiên', title_init: 'Độ ưu tiên', type: 'String', width: 50 },
      { name: 'startDate', order: 8, title: 'Ngày giao việc', title_init: 'Ngày giao việc', type: 'String', width: 50 },
...
const MenuFilter = React.memo(({ anchorEl, closeFilter, columns, selectField, filters, code }) => {
  if (code) {
    ...
    const data = list && list.listDisplay && list.listDisplay.type.fields.type.columns;
    const others = list && list.listDisplay && list.listDisplay.type.fields.type.others;
    const dataSearch = data && data.filter(item => item.type === 'String' && item.name === 'toBook');
    return (
...
ListPage.defaultProps = {
  ...
  filter: {},
  treeName: 'name',
  columnExtensions: [{ columnName: 'description', width: 300 }, { columnName: 'stt', width: 70 }],
  rightColumns: ['edit'],
  reload: false,
  perPage: 50,
  status: 'crmStatus',
