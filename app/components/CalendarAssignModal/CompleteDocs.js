...
import { API_MEETING, API_MEETING_APPROVE } from '../../config/urlConfig';
...
const DocumentAssignModal = props => {
  ...
  const handleSave = _docIds => {
    let url;
    url = docId && docId.length > 0 ? `${API_MEETING_APPROVE}` : `${API_MEETING}/set-complete`;
    const body = {
      docIds: calenDetail ? docId : docIds,
      processResult: resultProcess,
      saveResult: resultSave,
      calenderDetail: calenderDetailData,
    };
    request(url, {
      method: 'POST',
      headers: {
      ...
      .then(res => {
        setDisableSave(true);
        onChangeSnackbar({ variant: 'success', message: 'Phê duyệt thành công', status: true });
        if (props.onSuccess) {
      ...
      .catch(error => {
        setDisableSave(false);
        onChangeSnackbar({ variant: 'success', message: 'Phê duyệt thất bại', status: false });
      });
  };

  return (
    <DialogUI
      title="Đồng chí có chắc chắn muốn phê duyệt không ?"
      onSave={() => {
      ...
      saveText="Phê duyệt"
      disableSave={(!docIds || docIds.length === 0 || disableSave) && !saveAndSend}
