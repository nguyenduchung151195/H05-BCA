...
const DocumentAssignModal = props => {
  const { open, docIds, onChangeSnackbar, template, currentRole, typePage = '', isAuthory = false, handleComplete, id, calenDetail } = props;
  ...
  // console.log('docId',docId.length )
  ...
  return (
    <DialogUI
      title="Đồng chí có chắc chắn muốn hủy xuất bản không ?"
      onSave={() => {
        setDisableSave(true);
        let url;
        ...
        request(url, {
          ...
          .then(res => {
            setDisableSave(true);
            onChangeSnackbar({ variant: 'success', message: 'Hủy Xuất bản thành công', status: true });
            if (props.onSuccess) {
          ...
          .catch(error => {
            setDisableSave(false);
            onChangeSnackbar({ variant: 'success', message: 'Hủy Xuất bản kiến thất bại', status: false });
          });
      }}
      saveText="Hủy Xuất bản"
