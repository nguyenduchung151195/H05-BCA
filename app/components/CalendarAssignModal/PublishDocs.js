...
import { API_MEETING, API_MEETING_PUBLISH } from '../../config/urlConfig';
...
const DocumentAssignModal = props => {
  ...
  const resultsPro = JSON.parse(localStorage.getItem('crmSource')) && JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S42');
  const resultsSav = JSON.parse(localStorage.getItem('crmSource')) && JSON.parse(localStorage.getItem('crmSource')).find(item => item.code === 'S43');
  ...
  const handleSave = _docIds => {
    let url;
    url = docId && docId.length > 0 ? `${API_MEETING_PUBLISH}` : `${API_MEETING}/set-publish`;

    const body = {
      docIds: docId.length > 0 ? docId : docIds,
    ...
    request(url, {
      ...
      .then(res => {
        setDisableSave(true);
        onChangeSnackbar({ variant: 'success', message: 'Xuất bản thành công', status: true });
        if (props.onSuccess) {
      ...
      .catch(error => {
        setDisableSave(false);
        onChangeSnackbar({ variant: 'success', message: 'Xuất bản thất bại', status: false });
      });
    // if (handleComplete && id !== 'add') {
  ...
  return (
    <DialogUI
      title="Đồng chí có chắc chắn muốn xuất bản không ?"
      onSave={() => {
